...
### Step 1: 后端先跑通
后端开发按本指南的 **API 规范** 实现 FastAPI 服务，确认能独立运行并通过自测。
...
### 4.3 给后端开发的 Prompt 模板

前端可以直接复制以下内容发给后端开发，让其转发给 AI 助手（ChatGPT / Claude 等）：
...
| 角色 | 提交物 | 格式 |
|-----|-------|------|
| 后端开发 | 后端代码 | `main.py` + `requirements.txt` |
| 后端开发 | 自测截图 | API 文档页面 + 测试结果截图 |
| 后端开发 | 访问地址 | 完整 URL（含 IP 和端口） |
